...
First, RAII gives C++ *deterministic resource management*. Resources such as files, memory, locks, sockets, or GPU handles are released exactly when objects go out of scope. No garbage collector, no background cleanup - the program structure itself defines when cleanup happens.
...
For these reasons RAII became one of the defining idioms of C++ programming long before the modern standard library appeared. Developers routinely wrote small wrapper classes around files, sockets, mutexes, and other resources, relying on destructors to guarantee cleanup.
...
A `shared_ptr` manages a control block that stores the reference counters, deleter, and allocator information. As a result, using `shared_ptr` typically involves additional allocations and atomic operations.
